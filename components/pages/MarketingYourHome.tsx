...
const MarketingYourHome: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            <SEOHead
                title="Marketing Your Home in Connecticut | GayRealEstate.com"
            ...
            <header className="relative bg-gradient-to-br from-slate-800 via-slate-700 to-slate-900 pt-24 md:pt-32 pb-12 md:pb-24 overflow-hidden">
                ...
                <div className="absolute inset-0 md:hidden z-0 pointer-events-none">
                    <img
                        src="/images/marketing_hero_mobile.png"
                        alt="Premium Home Entry"
                        className="w-full h-full object-cover"
                    />
                    {/* Gradient only at the bottom to ensure text readability */}
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/80 to-slate-900/30" />
                </div>

                <div className="absolute inset-0 hidden md:block bg-[radial-gradient(circle_at_70%_40%,rgba(212,175,55,0.15),transparent_60%)]" />
                ...
                <div className="max-w-4xl mx-auto px-4 md:px-6 text-center relative z-10">
                    <div className="inline-flex items-center gap-2 px-4 py-1.5 md:px-5 md:py-2 bg-white/10 rounded-full border border-white/10 mb-6 md:mb-8">
                        <Share2 className="w-3.5 h-3.5 md:w-4 md:h-4 text-gold-400" />
                        <span className="text-[10px] md:text-xs font-bold text-gold-300 uppercase tracking-widest">Marketing Your Home</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif font-bold text-white mb-4 md:mb-6 leading-tight">
                        Maximum Exposure.<br />
                        <span className="bg-gradient-to-r from-gold-300 to-brand-300 bg-clip-text text-transparent">Maximum Result.</span>
                    </h1>
                    ...
                    <div className="flex flex-wrap gap-4 justify-center mt-10">
                        <a href="/#contact" className="px-8 py-4 bg-gradient-to-r from-brand-500 to-brand-700 text-white font-semibold rounded-full hover:shadow-lg hover:shadow-brand-500/30 transition-all inline-flex items-center gap-2">
                            List With Us <ArrowRight className="w-4 h-4" />
                        </a>
                ...
                <div className="max-w-6xl mx-auto px-4 md:px-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 text-center">
                        {[
                            ...
                            <div key={label}>
                                <p className="text-2xl md:text-4xl font-bold text-brand-600 mb-1">{n}</p>
                                <p className="text-[13px] md:text-sm font-semibold text-slate-800">{label}</p>
                                <p className="text-[11px] md:text-xs text-slate-400 mt-1">{sub}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="max-w-5xl mx-auto px-4 md:px-6 py-16 md:py-24 space-y-6 md:space-y-10">
                ...
                {strategies.map((s, i) => {
                    const Icon = s.icon;
                    return (
                        <div key={i} className="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 overflow-hidden group hover:border-brand-200 hover:shadow-xl transition-all duration-500">
                            <div className="p-5 md:p-8 flex flex-col md:flex-row gap-5 md:gap-8">
                                <div className="flex-shrink-0">
                                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center shadow-md shadow-brand-500/20 group-hover:scale-110 transition-transform">
                                        <Icon className="w-6 h-6 md:w-7 md:h-7 text-white" />
                                    </div>
                                </div>
                                <div className="flex-1">
                                    <h3 className="text-xl md:text-2xl font-serif font-bold text-slate-900 mb-2 md:mb-3 group-hover:text-brand-700 transition-colors">{s.title}</h3>
                                    <p className="text-slate-600 leading-relaxed mb-4 text-[13px] md:text-base">{s.description}</p>
                                    <div className="bg-gold-50 border border-gold-200 rounded-xl px-4 py-3 md:px-5 md:py-3 mb-4 md:mb-5 text-[13px] md:text-sm text-gold-800 font-medium italic">
                                        📊 {s.stats}
                                    </div>
            ...
            <section className="bg-gradient-to-br from-slate-800 to-slate-900 py-16 md:py-20">
                <div className="max-w-4xl mx-auto px-4 md:px-6 text-center">
                    ...
                    <p className="text-slate-400 text-xs md:text-sm mb-8 md:mb-12">— Seller, Glastonbury CT, 2025</p>
                    <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
                        <a href="/#contact" className="px-6 py-3.5 md:px-10 md:py-4 bg-gradient-to-r from-brand-500 to-gold-500 text-white font-bold rounded-xl md:rounded-2xl shadow-xl hover:shadow-brand-500/40 hover:-translate-y-0.5 transition-all duration-300 inline-flex items-center justify-center gap-2 text-sm md:text-base w-full sm:w-auto">
                            Start Your Listing Conversation <ArrowRight className="w-4 h-4 md:w-5 md:h-5" />
                        </a>
                        <a href="/home-valuation" className="px-6 py-3.5 md:px-10 md:py-4 bg-white/10 border border-white/20 text-white font-semibold rounded-xl md:rounded-2xl hover:bg-white/20 transition-all text-sm md:text-base flex justify-center w-full sm:w-auto">

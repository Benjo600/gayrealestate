  ...
  const renderCard = (agent: typeof allMembers[0], index: number) => {
    ...
    return (
      ...
      >
        ...
        <div className="absolute -inset-[1px] rounded-2xl md:rounded-[2.2rem] opacity-0 group-hover:opacity-100 blur-sm transition-opacity duration-700 bg-gradient-to-r from-gold-400/50 via-brand-400/50 to-gold-400/50" />
        ...
        <div className="relative glass-dark rounded-2xl md:rounded-[2rem] shadow-2xl backdrop-blur-2xl border border-white/5 hover:border-white/10 transition-all duration-500 overflow-hidden flex flex-col h-full">
          ...
          <div className="relative h-44 sm:h-56 md:h-80 overflow-hidden">
            ...
            <div className="absolute top-3 left-3 md:top-5 md:left-5">
              <div className="px-2 py-0.5 md:px-3 md:py-1 bg-black/30 backdrop-blur-md border border-white/10 rounded-lg">
                <span className="text-[9px] md:text-[11px] font-semibold tracking-widest text-white/70 uppercase">{badge.label}</span>
              </div>
            </div>

            {/* Name/Title overlay at bottom */}
            <div className="absolute bottom-0 left-0 right-0 p-3 md:p-6 pt-8 md:pt-12 bg-gradient-to-t from-black/60 to-transparent flex items-end justify-between">
              <div className="flex flex-col gap-0.5 md:gap-1">
                <h3 className="text-base md:text-2xl font-serif font-bold text-white mb-0 md:mb-1 group-hover:text-gold-200 transition-colors">
                  {agent.name}
                </h3>
                <p className="text-gold-300 font-medium text-xs md:text-base">{agent.title}</p>
                {(agent as any).instagram && (
                  <a
                    href={`https://instagram.com/${(agent as any).instagram.replace('@', '')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 md:gap-1.5 text-white/50 hover:text-white/80 transition-colors text-[10px] md:text-xs font-medium"
          ...
          <div className="p-3 md:p-6 flex-1 flex flex-col" style={{ background: 'linear-gradient(145deg, #fdf4ff 0%, #f0f4ff 50%, #f0fff8 100%)' }}>
            {/* Tagline */}
            <div className="relative mb-2 md:mb-4">
              <Quote className="absolute -top-1 -left-2 w-5 h-5 md:w-7 md:h-7 text-purple-100 -z-10" />
              <p className="text-slate-700 font-medium text-xs md:text-base leading-relaxed relative z-10 pl-2 not-italic">
                "{agent.tagline}"
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 md:gap-2 mb-2 md:mb-4">
              {agent.specialties.slice(0, 3).map((spec, i) => (
                <span key={i} className="px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[9px] md:text-xs font-medium" style={{ background: 'rgba(119,0,136,0.08)', border: '1px solid rgba(119,0,136,0.18)', color: '#5b006b' }}>
                  {spec}
                </span>
              ))}
            </div>
            ...
            <div className="mt-auto space-y-2 md:space-y-4">
              {/* Key Stat Row */}
              <div className="grid grid-cols-2 gap-2 md:gap-4 py-2 md:py-4" style={{ borderTop: '1px solid rgba(119,0,136,0.12)', borderBottom: '1px solid rgba(0,76,255,0.10)' }}>
                {/* Area/Award Stat */}
                <div className="flex items-center gap-1.5 md:gap-3">
                  <div className="w-7 h-7 md:w-9 md:h-9 rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(0,76,255,0.08)', border: '1px solid rgba(0,76,255,0.15)' }}>
                    <Target className="w-3 h-3 md:w-4 md:h-4" style={{ color: '#004CFF' }} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[9px] md:text-[10px] uppercase tracking-wider font-bold" style={{ color: '#004CFF99' }}>{agent.stats.find(s => s.label === 'Areas' || s.label === 'Award')?.label || 'Areas'}</span>
                    <span className="text-[10px] md:text-xs font-semibold text-slate-800 line-clamp-1">{agent.stats.find(s => s.label === 'Areas' || s.label === 'Award')?.value || 'Top Performer'}</span>
                  </div>
                </div>

                {/* Experience Stat */}
                <div className="flex items-center gap-1.5 md:gap-3">
                  <div className="w-7 h-7 md:w-9 md:h-9 rounded-lg md:rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(2,129,33,0.08)', border: '1px solid rgba(2,129,33,0.15)' }}>
                    <Briefcase className="w-3 h-3 md:w-4 md:h-4" style={{ color: '#028121' }} />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[9px] md:text-[10px] uppercase tracking-wider font-bold" style={{ color: '#02812199' }}>Experience</span>
                    <span className="text-[10px] md:text-xs font-semibold text-slate-800 line-clamp-1">{agent.stats.find(s => s.label.includes('Experience') || s.label.includes('Service'))?.value || 'Experienced'}</span>
                  </div>
                </div>
              </div>
  ...
  return (
    <section id="find-agent" className="py-12 md:py-24 lg:py-32 relative overflow-hidden" style={{ background: 'linear-gradient(160deg, #fdf4ff 0%, #fff7f0 25%, #f0f9ff 50%, #f7fff4 75%, #fdf4ff 100%)' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Top Row: 3 Agents */}
        <div className="grid md:grid-cols-3 gap-4 md:gap-8 lg:gap-10 mb-4 md:mb-10">
          {topRow.map((agent, index) => renderCard(agent, index))}
        </div>

        {/* Bottom Row: Jake (Lender) and Carolyn (Attorney) side by side */}
        <div className="grid md:grid-cols-2 gap-4 md:gap-8 lg:gap-10 max-w-4xl mx-auto">

/**
 * Fix corrupted excerpts (JS String.replace treats $N as backrefs).
 * Also re-assert titles + keywords for safety.
 */
const fs = require('fs');
...
function setField(block, key, value) {
  ...
  // Escape $ for String.replace so $8,000 isn't treated as a capture group
  // Function replacer avoids $N backref issues when value contains $8,000 etc.
  const escaped = value.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  return block.replace(re, (_m, p1, _old, p3) => p1 + escaped + p3);
}
...
for (const [slug, fields] of Object.entries(FIXES)) {
  ...
  const end = next === -1 ? endArr : Math.min(next, endArr);
  let block = src.slice(start, end);
  for (const [k, v] of Object.entries(fields)) {
    block = setField(block, k, v);
  }
  src = src.slice(0, start) + block + src.slice(end);
  console.log('fixed', slug, 'excerptLen', fields.excerpt.length, 'pageTitle', `${fields.title} | Gay Real Estate CT`.length);
}

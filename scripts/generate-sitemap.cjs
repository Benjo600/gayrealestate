...
try {
  ...
  const edgeOut = path.join(ROOT, 'netlify', 'edge-functions', '_content-meta.ts');
  const fileBody =
  ...
  console.log(
    `[sitemap] wrote netlify/edge-functions/_content-meta.ts (${blogMeta.length} blogs, ${Object.keys(agentMeta).length} agents).`
  );
} catch (e) {
